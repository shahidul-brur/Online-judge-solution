...
int main()
{
	...
	while(t--)
	{
		...
		int odd = 0, ch = -1;
		f(i, 0, 25) if(freq[i]%2==1) odd++, ch = i;
		if(odd>1)
			cout << "-1\n";
		else
		{
			int l = 0, r = n-1;
			vector<int> pos[26];
			f(i, 0, n-1) pos[s[i] - 'a'].pb(i+1);
			f(i, 0, 25) 
			{
				if(i==ch) continue;
				f(j, 0, freq[i]/2-1) ans[l++] = pos[i][j];
				f(j, freq[i]/2, freq[i]-1) ans[r--] = pos[i][j];
			}
			if(ch!=-1)
				f(j, 0, freq[ch]-1) ans[l++] = pos[ch][j];
			cout << ans[0];
