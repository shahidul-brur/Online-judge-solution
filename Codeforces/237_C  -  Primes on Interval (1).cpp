...
void sieve()
{
	isComposite[1] = 1;
	int r = sqrt(MAX);
	for(ll i=4;i<=MAX;i+=2) isComposite[i] = 1;
	for(ll i=3;i<=r;i+=2) if(!isComposite[i])
		for(ll j = i*i;j<MAX;j+=2*i)
			isComposite[j] = 1;
	for(int i = 2;i<MAX;i++)
		cnt[i] = cnt[i-1] + !isComposite[i];
}
int main()
{
	...
	int ans = R-L+1;
	for(++R;R<=b;R++){
		while(L<R && cnt[R]-cnt[L]>=k && R-L>=ans)
				++L;
		ans = max(ans, R-L+1);
	}
	cout << ans << "\n";
