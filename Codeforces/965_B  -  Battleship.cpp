...
int main()
{
    ...
    while(cin>>n>>k)
    {
        ...
        f(i, 1, n)
        {
            f(j, 1, n)
            {
                if(j+k-1<= n && rowDot[i][j+k-1] - rowDot[i][j-1] == k)
                {rCnt[i][j]++; rCnt[i][j+k]--;}
                if(i+k-1<= n && colDot[j][i+k-1] - colDot[j][i-1] == k)
                {cCnt[j][i]++; cCnt[j][i+k]--;}
            }
        }
        f(i, 1, n)
