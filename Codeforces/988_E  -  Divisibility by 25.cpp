...
int swap_count(string last)
{
    ...
    f0(i, 2)
    {
        fr(j, len-1-i, 0)
        {
            if(s[j]==last[i])
            {
                cnt+=len-i-1-j;
                //cout << j << " -> " << len-i-1 << " : " << cnt << "\n";
                f2(k,j,len-1-i-1) swap(s[k], s[k+1]);
                break;
            }
        }
    }
    if(s[len-1]!=last[0] || s[len-2]!=last[1])
    {
        return INT_MAX;
    }
    reverse(all(last));
    if(s==last)
        return cnt;
    f0(i, len-2) if(s[i]!='0') return cnt+i;
    return INT_MAX;
}
int main()
{
    ...
    cin>>str;
    len = str.sz;
    int ans = swap_count("00");
    //cout << ans << " ";
    ans = min(ans, swap_count("25"));
    //cout << ans << " ";
    ans = min(ans, swap_count("50"));
    //cout << ans << " ";
    ans = min(ans, swap_count("75"));
    //cout << ans << " \n";
